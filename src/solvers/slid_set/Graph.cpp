...
void Graph::getEdges(std::vector<std::pair<std::pair<int, int>, int> >& edge_vec) {
    adjacency_list::vertex_descriptor  v1, v2;
    std::pair<edge_iterator, edge_iterator> ep;
    edge_iterator ei, ei_end;
    for (tie(ei, ei_end)=edges(adj_list); ei!=ei_end; ++ei) {
 ...
 */
/* --------------------------------------------------------------------------*/
int Graph::getEdgeProperty(size_t u, size_t v) const
{
...
int Graph::getEdgeProperty(edge_descriptor e)
{
    return getEdgeProperty(source(e), target(e));
}
/* --------------------------------------------------------------------------*/
/**
 * @synopsis  seekCc calculate the connected components
 */
/* --------------------------------------------------------------------------*/
void Graph::seekCc()
{
...
/* --------------------------------------------------------------------------*/
/**
 * @synopsis  seekCycle calculate the cycle of the Graph
 */
/* --------------------------------------------------------------------------*/
void Graph::seekCycle()
{
                    ...
                    for (; it1 != it2; ++it1) {
                        ...
                        if (it != end(s)) {
                            ...
                            for (int idx=0; idx+1<t1.size(); idx++) {
                                ...
                                edge.second = getEdgeProperty(t1[idx], t1[(idx+1)]);
...
std::vector<int> Graph::getCcCycleNum() const
{
...
std::vector<Graph::cc_cycle_t>& Graph::getCcCycle()
{
    return cycle;
}

int Graph::whichCc(size_t v) const
{
...
std::pair<int, int> Graph::getCycleCoord(edge_descriptor e) const
{
    return getCycleCoord(boost::source(e, adj_list));
}

bool Graph::isCycle(const std::pair<int, int>& coord) const
{
    return (coord.first >= 0 && coord.second >= 0);
}

std::vector<int> Graph::getCycle(const std::pair<int, int>& coord) const
{
    assert(coord.first < cycle.size() && coord.second < cycle[coord.first].size());
    return cycle[coord.first][coord.second];
}

std::vector<Graph::edge_t> Graph::getEdgeCycle(const std::pair<int, int>& coord) const {
    assert(coord.first < edge_cycle.size() && coord.second < edge_cycle[coord.first].size());
    return edge_cycle[coord.first][coord.second];
}

std::vector<Graph::edge_descriptor> Graph::mergePath(std::vector<Graph::edge_descriptor>& path1, std::vector<Graph::edge_descriptor>& path2)
{
...
std::vector<Graph::edge_descriptor> Graph::mergePath(std::vector<Graph::edge_descriptor>& path1, std::vector<int>& c) const
{
    std::vector<edge_descriptor> res(path1);
...
std::vector<std::pair<int, int>> Graph::getCycleCoords() const
{
...
std::vector<Graph::edge_descriptor> Graph::getPath(size_t u)
{
    std::vector<edge_descriptor> res;
    std::pair<int, int> coord = getCycleCoord(u);
    if (!isCycle(coord))
        return res;

    std::vector<int> c = getCycle(coord);
    out_edge_iterator ei, ei_end;
    std::vector<int>::iterator it, it2;
...
std::set<size_t> Graph::getReachableSet(size_t u) {
    std::vector<edge_descriptor> res;
...
std::vector<Graph::edge_descriptor> Graph::getReachableEdges(size_t u) {
    std::vector<edge_descriptor> res;
...
std::vector<Graph::edge_descriptor> Graph::getPath(size_t u, size_t v) const
{
    ...
    for (size_t i = 0; i < t.size(); ++i)
        m[t[i]] = i;
    return getPath(u, v, s, visited, m);
}

std::vector<Graph::edge_descriptor> Graph::getPath(size_t u, size_t v,
                                                    ...
                                                    std::map<int, int>& m) const
{
    ...
    for (; ei != ei_end; ++ei) {
        ...
        if (!visited[m[targ]]) {
            s.push_back(*ei);
            res = getPath(targ, v, s, visited, m);
            if (!res.empty())
                break;
            s.pop_back();
        }
...
std::pair<Graph::edge_iterator, Graph::edge_iterator> Graph::getEdge()
{
    return boost::edges(adj_list);
}

size_t Graph::getVertexId(size_t vid)
{
