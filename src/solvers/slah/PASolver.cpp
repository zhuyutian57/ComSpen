...
//std::cout<<"ex_var_set: " <<ex_var_set<<std::endl;
		...
		//system("read REPLY");
			...
			try{
		    	...
		    	pre_match(absA, spaceA, absB, spaceB, ex_var_set, pi_ex_set);//��Ϊ����match_atomʱ��Ҫȫ����absA�����ֻ��dataA�ͻᶪʧһЩԼ������absB����Ϊ����������д��ڱ���ȡֵ��Լ������Ҫ�������̺��Ƿ���� 
...
z3::expr_vector PASolver::get_conjunct(z3::expr formula){
	...
	if(!is_fun(formula, "and")){//disjunct�п�����ԭ�ӻ�ռ乫ʽ 
 ...
 */
z3::expr PASolver::pred2abs(z3::expr &atom) {//solver�ж���Ĵ��麯�� 
	
    //if (atom.to_string() == "emp") return z3_ctx.bool_val(true);//emp��CommandParser::_parseExpr()�б������� 
...
void PASolver::pre_P(z3::expr spaceA, z3::expr dataA, z3::expr spaceB, z3::expr dataB, z3::expr absA, z3::expr spaceAhls, z3::expr spaceBhls, z3::expr_vector &ex_var_set, z3::expr_vector &pi_ex_set) throw (int){
	...
	if(spaceA.num_args() != 0){
		...
		if(isP(atom)){
			...
			if(rel.get_relation(source,dest) == 0 || rel.get_relation(source,dest)==1 || rel.get_relation(source,dest)==3){//z1 ? z2 ���� z1 = z2  ���� z1 <= z2 
				z3_sol.reset();
    		...
    		if(rel.get_relation(source,dest) == 0 || rel.get_relation(source,dest) == 2|| rel.get_relation(source,dest) == 3){//z1 ? z2 ���� z1 < z2 ���� z1 <= z2 
    			z3_sol.reset();
...
//start = clock();
	...
	for(int i = 0; i < adds.size(); i++){
		...
		for(int j = 0;j < adds_rep.size(); j++){
			if(rel.get_relation(adds[i],adds[j])==1){
    			...
    			addsuc = true;
    			break;
			}else if(rel.get_relation(adds[i],adds[j])!=2 && rel.get_relation(adds[i],adds[j])!=4){//���� < �� >
				z3_sol.reset();
	...
	for(int i = 0; i < adds_rep.size(); i++){
		for(int j = 0; j < adds_rep.size(); j++){
			...
			}else if(rel.get_relation(adds_rep[i],adds_rep[j]) == 3){//�ʼ��<=������hls��д���ǿ��ܱ��<�� 
				z3_sol.reset();
...
z3::check_result PASolver::check_entl_get_sorts(z3::expr_vector &adds_rep, Relation &rel_sub, z3::expr &absA, std::vector<z3::expr_vector> &adds_eq,z3::expr &dataA, z3::expr &spaceA, z3::expr &dataB, z3::expr &spaceB, z3::expr_vector &ex_var_set, z3::expr_vector &pi_ex_set){
	...
	std::vector<std::vector<z3::expr_vector>> sort_set_tmp;//��sort��ɣ�ÿ��sort��z3::expr_vector��ɣ���һ��z3::expr_vector�е�ַ����� 
	...
	while(!sort_set_tmp.empty()){
		...
		for(int i = 0; i < adds_rep.size(); i++){
			...
			if(get_eq_class(sort_tmp,add).size() ==0){//add�������sort_tmp��
				for(int j =0;j<adds_rep.size();j++){
			...
			if(flag_tmp[i] == 1){
				...
				if(!is_less){
					if(!is_in_sortset(sort_set_tmp,sort_tmp_new1)){//{x1,t1} and {t1,x1}, avoid duplication
						...
						for(int j = 0;j<adds_rep.size();j++){//�ѵ�ǰ���ӵĵ�ַ<=���в������sort�е�add�������Ҳ���� 
...
//system("read REPLY");
							...
							}else{//sort is incomplete��Ϊ�˷�ֹ{x1,t1} and {t1,x1}�����ظ������뵽ǰ��ȽϺ� 
