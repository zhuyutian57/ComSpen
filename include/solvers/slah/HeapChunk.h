...
private:
        ...
        int m_z_size;//��check_preds()�и�ֵΪ����ν�ʵ��д�������z'�ĸ��� 
        int m_ex_size;//parseʱ��ü�w�ĸ��� 
        int m_v_size;
        Fts m_fts;
        
        int type;// 0: true or n<=y-x  1: n<=y-x<=n'  2: n<=y-x<=vj
        int v_index;//��set_v�и�ֵ 
        int min_bound;
        int max_bound;
        std::vector<int> v_index_set;//ʵ����Ӧ�ô治������sigma�У�ֻ������piԼ���е�index ��������� 
public:
z3::context& z3_ctx;
HeapChunk(z3::context& ctx, z3::func_decl fun, z3::expr_vector pars, z3::expr exp, int ex_size);
        ...
        void get_summ(z3::expr &summ,int &k_index,z3::expr_vector args);//�����滹��ȥ����v_index,v_index_set,v_min_bound 
